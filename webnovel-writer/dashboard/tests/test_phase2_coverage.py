...
def test_root_fallback_page_points_to_canonical_repo_commands(monkeypatch, tmp_path):
    from dashboard import app as dashboard_app

    missing_static_dir = tmp_path / "missing-dist"
    monkeypatch.setattr(dashboard_app, "STATIC_DIR", Path(missing_static_dir))

    app = dashboard_app.create_app(project_root=None)
    with TestClient(app) as c:
        resp = c.get("/")

    assert resp.status_code == 200
    body = resp.text
    assert "running/init.ps1 -ProjectRoot" in body
    assert "webnovel-writer/scripts/webnovel.py dashboard --project-root" in body

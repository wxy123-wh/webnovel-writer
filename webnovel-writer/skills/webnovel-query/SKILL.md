...
- **禁止**在插件目录 `${CODEX_PLUGIN_ROOT}/` 下读取或写入项目文件

环境设置（bash 命令执行前）：
```bash
export WORKSPACE_ROOT="${CODEX_PROJECT_DIR:-$PWD}"

if [ -z "${CODEX_PLUGIN_ROOT}" ] || [ ! -d "${CODEX_PLUGIN_ROOT}/skills/webnovel-query" ]; then
  echo "ERROR: 未设置 CODEX_PLUGIN_ROOT 或缺少目录: ${CODEX_PLUGIN_ROOT}/skills/webnovel-query" >&2
  exit 1
fi
export SKILL_ROOT="${CODEX_PLUGIN_ROOT}/skills/webnovel-query"
